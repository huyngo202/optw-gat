...
\begin{itemize}
    \item \textbf{Nhóm 1: Đặc trưng tương đối với thời gian hiện tại ($t_{curr}$):}
    \begin{enumerate}
        \item Thời gian chờ đến khi mở cửa: $(O_v - t_{curr}) / T_{max}$.
        \item Thời gian còn lại đến khi đóng cửa: $(C_v - t_{curr}) / T_{max}$.
        \item Thời gian còn lại của Service Time hoặc đại lượng liên quan tại node: $(ArrivalIndex_v - t_{curr}) / T_{max}$.
        \item Tỷ lệ thời gian đã trôi qua của tour: $(t_{curr} - T_{start}) / T_{max}$.
    \end{enumerate}
    \item \textbf{Nhóm 2: Đặc trưng tương đối với thời gian dự kiến đến nơi ($t_{arrival} = t_{curr} + \Delta_{uv}$):}
    \begin{enumerate}
        \item Thời gian từ lúc xuất phát đến khi đến nơi: $(t_{arrival} - T_{start}) / T_{max}$.
        \item Thời gian đến sớm hơn giờ mở cửa: $(O_v - t_{arrival}) / T_{max}$.
        \item Thời gian dư trước giờ đóng cửa: $(C_v - t_{arrival}) / T_{max}$.
        \item Thời gian Service Time so với thời gian đến: $(ArrivalIndex_v - t_{arrival}) / T_{max}$.
    \end{enumerate}
\end{itemize}
...
\begin{algorithm}[H]
\SetAlgoLined
\KwData{Coordinates $X$, Service Times $D$, Time Windows $[O, C]$, Scores $S$, Beam Width $k$}
\KwResult{Best Route $\pi^*$}
 \textbf{Initialize:} Beam $B_0 = \{(\text{depot}, \text{score}=0, t=0)\}$; \\
 \For{step $t = 1 \dots N$}{
  $Candidates \leftarrow \emptyset$\;
  \For{each hypothesis $h \in B_{t-1}$}{
    Update dynamic features for current state $h$\;
    Compute Mask $M$: Check Time Windows, Lookahead, Visited\;
    Compute Probabilities $P$ using GAT-Transformer Policy excluding masked nodes\;
    Select top $k$ next nodes based on $P$\;
    Add extended hypotheses to $Candidates$\;
  }
  $B_t \leftarrow$ Select top $k$ hypotheses from $Candidates$ with highest cumulative score\;
  \If{all hypotheses in $B_t$ are finished}{
   \textbf{break}\;
  }
 }
 $\pi^* \leftarrow$ Hypothesis in $B_t$ with max score\;
 \caption{Quy trình Suy diễn Beam Search có kiểm soát ràng buộc (Feasibility Masking)}
